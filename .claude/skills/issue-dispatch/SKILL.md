...
**Scope に設計書を書くときは、台帳（`.claude/docs/SOURCES.md` §2）の持ち主と `lane:` を揃えます。** 持ち主でないレーンに書かせる Issue は立てません。正本どうしの食い違いは題を「正本の食い違い: <何と何が>」で始め、両方の `file:§` と、直し方ごとに変わる結果を書きます。書く側が決まるまで `lane:` は付けません（#116 が型）。
