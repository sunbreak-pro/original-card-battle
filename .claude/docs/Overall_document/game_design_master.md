...
| 間合い      | 横 1 列のマスの上の距離 N。届く間合いと境目で読み、どの間合いで終えるかを選ぶ | battle_core_v4 §7 / §21 |
...
- 間合いは横 1 列のマスの上の距離 N（あいだの空きマスの数）。マスの数は層と戦闘ごとに変わり、開始の間合いは 3 が既定（2026-09-23）。全ての敵がマスを持って自分で動き、1 つのマスに敵は 1 体（取り巻きと群れは置かない）。カードは届く間合いと 1 つの境目で N を読み、押す / 引くと壁のダメージがある（v4.3、2026-09-22。`battle_core_v4.md` §7）。
...
- 敵の頁: 弱点 / 耐性 / 行動 / 間合いの動かし方（届く間合い・狙うマス）/ 特殊行動 / 条件 / 関係 / 適応条件。
...
- 敵の行動には予兆・条件・傾向・適応条件がある。全ての敵がマスを持って間合いを動かし、予兆は種別と狙うマスで示す。届く間合いの外なら空振りになる。
...
| 1     | 戦闘コア v4.3（固定コスト・間合い N・敵の位置戦術）    | R1-2〜R1-4        |
