...
- **遺産**: 前回死亡者だけ。技術 / 習熟カードの成果 / 敵知識 / 探索知識 / 特殊能力から数個（仮 3）を選ぶ。浅層・弱い敵での死亡では高習熟カードは残らない。
...
| V3.1 の施設      | V4.0                                                    | 状態   |
| ---------------- | ------------------------------------------------------- | ------ |
| ギルド           | **継承の間**（遺産選択 / 生存者 / 新キャラ開始）        | 再解釈 |
| ダンジョンゲート | **出立**（ツール / 戦闘デッキ / ステージ / 出発）       | 再解釈 |
...
- 数値は `battle_document/`（改訂は tier1 R1-2）。
...
- 難易度選択は無い。ステージ（または階層）の選択が難易度になる。
...
- `battle_document/`（tier1 R1-2 で改訂予定。現行 V4.0 の AP 節は凍結）
