# Plan: 戦闘コア — ゲームエンジン Bake-off（PixiJS版 × Phaser 4版）

> **Note（2026-06-28 実装時）**: 導入された Phaser は **v4.2.0**（計画当初の「Phaser 3」表記から更新。Phaser 4 の API で実装）。
> **Status**: IN PROGRESS — Phase 0-3 完了 / Phase 4（エンジン選定）はユーザー実機プレイ待ち
> **Created**: 2026-06-28
...
> **状態（2026-06-28）**: Phase 0-3 実装完了・検証通過（tsc / test 203 / build 4エントリ green、session-verifier PASS + 独立 QA PASS-with-fixes・Blocker 0）。エンジン選定はユーザー実機プレイ待ち。
>
> 採用エンジン: （未定 — ユーザー実機プレイ後に確定）
> 客観指標の傾き: 既存統合・バンドル・カード UI・発展性は PixiJS 優位 / 演出の手軽さは Phaser 優位（比較評価表 参照）。肌感（最重要）とパフォーマンスはユーザー判断。
> 理由: （未定）
> 負けたアダプタの扱い: （未定 — 計画方針は `archive/`。コアは選定結果に関わらず存続）
