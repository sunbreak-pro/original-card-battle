...
- 現在: **要件正本化＋Bake-off Phase 0 prep を完了**。origin/main（`d4a57fd`）に Tier1/2/3 正本化・umbrella削除・bake-off計画書を反映。専用 worktree `../battle-bakeoff`（`feat/battle-engine-bakeoff`、origin push 済）を作成し rollup時限爆弾除去(#5)＋phaser導入(v4.2)を独立コミット（`117e8b1`/`fd29c11`、build・test156件green・脆弱性0）。**※rollup除去は feat のみ。main の package.json はまだ time-bomb 残**（feat マージ時に解消 or 要 cherry-pick）。**※phaser は最新 v4 導入、計画書記載「3」と差異 — v3 固定するか要確認**
