...
- 🔜 **戦闘描写の後片付け（着手可能）** — 2026-09-19 の描写セッションで範囲の外に出した点と、QA で見送った点。Claude だけで進められる 4 件をまとめる: `BattleScreenView` の自動起動にシーンの条件（#26）/ `unity:sync` が改行だけの差分を作らない（#27）/ 1 行動 2.0 秒の PlayMode テスト（#28）/ 台本に攻撃者と一字札の文字を持たせる（#29 前半）。プランと冒頭プロンプトは `vision/plans/2026-09-19-depiction-followups.md`。前提は PR #24 と RPG-by-card#1 のマージ。**人手待ち**: 手触りの確認（#25）。**設計待ち**: 強弱 4 段のしきい値と投げ上げ線の予測値の形（#30、v4.2 / v2.1 の改訂で決める）。#29 の後半（v4.2 コアからの変換器）はコアの C# 実装の後
