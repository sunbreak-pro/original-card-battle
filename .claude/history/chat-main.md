...
こうだいさんの決定「竜が全ての敵の親玉。敵の多くは竜・亜竜・竜の系譜に絞る」と「世界観の持ち主を design から main へ」を、正典と規約と Issue に反映した。

#### 変更点

- **正典（#127、PR #131 open）**: `world-v1.md` を v4 にした。§0 持ち主は main / §2 竜の系譜 4 段（竜神・竜・亜竜・眷属 = 竜人と小竜）と例外の上限（歪みの根を除いて 2 体）/ §7 命名を系譜前提へ / §8 歪みの根は竜神が伸ばしているものと確定。design レーンの v3（PR #126）を土台にしたので、#131 は #126 の commit を含む
- **持ち主の付け替え**: `.claude/CLAUDE.md` のレーン表、`worktree-policy`、`issue-dispatch`、`issue-prompter`、`concept-v3.md` §17。`area:world` は `lane:` を付けず main の采配に落ちる
- **Issue**: 起票 #128（ロースターを系譜へ。姿と名前だけ、行動表と数値は据え置き）/ #129（出現表を七層へ、大物はセルク・ガルド・根・竜神）/ #130（濃度と刻限を七層へ）。#39 を「竜神」に改題。#83・#85〜#88（#128 待ち）・#67・#70（止めずに進める）・#111 にコメント。Close は 0 件
- **置いた仮定**: 眷属を竜人と小竜の 2 種にした（現行の行動表を数値ごと流用するため）。見た目の約束（色・光源）も `world-v1.md` に残し、書き手を main にした
- **作業場所**: worktree `workspaces/original-card-battle/world`。merge 後に消せる

### 2026-09-21 - Issue 46 件の起票、優先順位 4 段階の仕組み、未コミットの PR 化

#### 概要

敵を 1 体作ってターン終了まで通す縦切りと、ゲームに要る残りの領域を Issue にした。起票のたびに優先順位を付ける仕組みを入れ、メインに溜まっていた未コミットを PR 3 本に分けて出した。

#### 変更点

- **起票（#66〜#111）**: 縦切り「長柄の歪み兵と 1 ターンを通す」親 #67 + 子 13、アート 親 #81 + 子 12、探索と世界観 親 #94 + 子 9、生と継承 親 #104 + 子 7、仕組み #66。起票前に質問で範囲を確かめた（縦切りは新規の親子で起こす / 最初の敵は長柄の歪み兵 / 演出は札・攻防・状態とターン・計測の全部 / 状態チップはプレイヤー側の札で試す）
- **優先順位（#66、PR #112 merged）**: ラベル `prio:1`〜`prio:4`。`scripts/issues-next.mjs`（`npm run issues:next`、`--all` / `--lane` / `--check` / `--json`）、node:test 7 件、Issue テンプレート 3 種の必須ドロップダウン、`.github/workflows/issue-priority.yml`、`issue-dispatch` / `issue-prompter` / `session-loader` / `.claude/CLAUDE.md` の規約。既存の open 23 件に `prio:` を付けた
- **未コミットの PR 化**: 退避コミット `5805607`（ブランチ `wip/main-uncommitted-20260921`、ローカルのみ）で全体を保全し、origin/main の上に載せ直した。#113 = v4.2 の正本改訂 + 計画書、#114 = アートの仕様カード + Krita の引き継ぎ書 + レポート 2 件、本 PR = tracker。メインのチェックアウトは `main` に戻した
- **#113 の統合で判断した点**: 試験台のシナジー基準を 13〜17 に振り直し（§19 の本文は未変更）/ デッキの型を 6 型へ / ボス固有カード #44〜49 は名前が #64・数値が v4.2 / 「状態 4 枠」の 2 段落を種類 6 つの規則へ書き換え、試験台の項目 17 は入れず / §19 は v4.1 の語のまま
- **気づき**: `briefs/polearm_warped.md` は 5 行動、正本は 4 行動（#85 に記載）。旧 `MEMORY.md` への追記は main でポインタ化済みだったので捨て、内容は本ファイルと memory に移した
...
- **PR**: #113（2026-09-21 に main の #63 / #64 と統合して提出）。アートの仕様カードと Krita の引き継ぎ書は #114

### 2026-09-20 - 開発環境を life-editor から移植

#### 概要

並行チャットの衝突対策と Issue 駆動の運用を life-editor から持ち込んだ。hook・per-chat タスク管理・Issue テンプレートとラベルまでを入れ、worktree と CI は次の層に回した。

#### 変更点

- **hooks**: `.claude/hooks/` に 5 本（`regen-index` / `session-start-check` / `pre-commit-mcp-check` / `pre-commit-index-guard` / `pre-commit-tracker-guard`）。同梱 fallback を `.claude/scripts/hooks-lib/` に 4 本。実体は `$HOME/dev/Claude/hooks-lib/` を優先する
- **settings.json 新設**: SessionStart 2 本 + PreToolUse(Bash) 3 本。`deny` 18 件（main への push / force push / `reset --hard` / `branch -D` / `rm -rf .git`）、`ask` に `gh pr merge*`。worktree 用の `git checkout main` deny は未導入（共用ブランチが止まるため）
- **per-chat 化**: `MEMORY.md` → `memory/chat-main.md`、`HISTORY.md` → `history/chat-main.md`。旧 2 ファイルは後方互換のポインタに差し替え。`.claude/comm/.session-name` = `main`、`.session-branch` = 現ブランチ
- **gitignore**: `memory/INDEX.md` `history/INDEX.md` `.session-name` `.session-branch` `active-sessions/` を追加
- **Issue 駆動**: `.github/ISSUE_TEMPLATE/` 3 種（Known Issue / Roadmap Item / Human Task）。ラベル 19 件（`type:` `sev:` `area:` `status:`）を作成。`section:` は worktree 未導入のため見送り
- **CLAUDE.md**: 「Task Completion Rule」を「Development Workflows」に置き換え、Issue 運用 / per-chat / hooks 一覧 / 禁止コマンド / 未導入の明示を追加

#### 実測・知見

- hook 3 本を実行して確認。`pre-commit-tracker-guard` は tracker 同梱で exit 2、`[tracker-ok]` で通過。`pre-commit-index-guard` は `INDEX.md` を自動 unstage して他を残す。`regen-index` は INDEX 2 本を生成
- `session-start-check.sh` が Windows で落ちる。共有 hooks-lib の `stat -f %m` が GNU stat では stdout にファイルシステム情報を出すため、コマンド置換にゴミが混ざり `set -u` で落ちる。`|| true` で暫定回避し #40 に起票
- ラベルは `gh label create --force` で 19 件とも成功

#### 次

4 件の Issue を起票済み（#36 手札の縛り / #37 AI っぽさ / #38 ボス設計 / #39 竜）。依存順は #36 → #37 → #38 → #39。
