...
- **実装の正:** 戦闘コアは `unity-port/BattleCore/`（C#）が正（2026-09-12）。TS の `src/ui/battle-lab/core/` は凍結。戦闘の規則は `docs/battle_document/battle_core_v4.md` v4.3（2026-09-22、横 1 列のマスの間合い N）で、コアの追従は #162
- **戦闘描写（2026-09-20）:** 画面は `unity-port/unity-project-kit/Assets/View/Depiction/`（`Script/` は純 C# の台本と型、View は名前空間 `Depiction.View`）。C# はこのリポが正で `npm run unity:sync` で写す。プレハブ / シーン / `.meta` は Unity リポが正（`Assets/Prefabs/Depiction/`、撮影用の `Assets/Scenes/BattleDepiction.unity`、縦切りの実戦用 `Assets/Scenes/Battle.unity`）。**View は計算せず、値は全て台本から取る**。台本を出すのは `IDepictionSource` の実装 3 つで、既定は実戦の `LiveTurn`（手札から好きな札を出せる）、撮影用は固定台本の `DepictionRunner`（`DepictionPlayer.scriptedPlayback`）、v4.2 のコアで戦うのは `Bridge/` の `CoreBattleSource`（`BattleCore.TurnLoop` のイベント列を `CoreScriptWriter` が台本へ写す。BattleCore と台本の型の両方を見るのは `Bridge/` だけ。`Battle.unity` では `BattleBootstrap` が Inspector の敵 id と乱数の種を `BattleLaunch` へ写し、`DepictionPlayer.UseSource` へ渡す）。`Script/` `Bridge/` と EditMode テストは `unity-port/Depiction.Script*.csproj` `Depiction.Bridge*.csproj` 経由で `dotnet test` が回す
