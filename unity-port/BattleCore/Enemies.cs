using System;
using System.Collections.Generic;

namespace BattleCore
{
    /// <summary>
    /// Enemy data, written as records from enemy_document/enemy_roster_v4.md. Nothing here decides
    /// anything: the decision tree is walked by <see cref="EnemyAi"/> and the numbers are resolved by
    /// the turn loop (#72).
    ///
    /// The slice carries the first normal enemy only (roster §2.1). #51 widens <see cref="All"/> to
    /// the other eleven without changing the record shape — every row below is an
    /// <see cref="EnemyActionDef"/> written off the same face table the cards use (§6.1).
    /// </summary>
    public static class Enemies
    {
        /// <summary>The roster id. Renaming it (and the display name) waits for #128.</summary>
        public const string PolearmWarpedId = "polearm_warped";

        /// <summary>
        /// roster §2.1: 長柄の歪み兵. HP 60 / max stamina 10 / recovery 2 / no position / punishes the far
        /// side. Four actions, a two-branch tree, a one-step omen, no statuses.
        ///
        /// The omen labels are the roster table's, authored and not derived: 石突きの押し込み reads no
        /// side (its trait is 無防備) yet the table says 攻撃・近. That mismatch is #116 and is carried
        /// here as written.
        /// </summary>
        public static readonly EnemyDef PolearmWarped = Build(
            PolearmWarpedId,
            "長柄の歪み兵",
            maxHp: 60,
            maxStamina: 10,
            recovery: 2,
            branchWhenPlayerNear: new[] { "shove", "guard_up", "reach_thrust" },
            branchWhenPlayerFar: new[] { "sweep", "reach_thrust", "guard_up" },
            actions: new[]
            {
                new EnemyActionDef(
                    "sweep", "薙ぎ払い", BattleAttribute.Attack, 2,
                    new Face(Power: 8),
                    new OmenLabel(OmenKind.Attack, Position.Far),
                    new Trait(
                        TraitCondition.OpponentPosition, TraitEffect.PowerBonus,
                        Amount: 3, ConditionPosition: Position.Far),
                    "位置（相手 遠間）: 威力 +3"),

                new EnemyActionDef(
                    "shove", "石突きの押し込み", BattleAttribute.Attack | BattleAttribute.Move, 2,
                    new Face(Power: 5, Push: true),
                    new OmenLabel(OmenKind.Attack, Position.Near),
                    new Trait(TraitCondition.Unguarded, TraitEffect.PowerBonus, Amount: 3),
                    "相手の位置を反転する。無防備: 相手の Guard が 0 なら威力 +3"),

                new EnemyActionDef(
                    "reach_thrust", "穂先の突き", BattleAttribute.Attack, 1,
                    new Face(Power: 4),
                    new OmenLabel(OmenKind.Attack)),

                new EnemyActionDef(
                    "guard_up", "柄で受ける", BattleAttribute.Guard, 1,
                    new Face(Guard: 3),
                    new OmenLabel(OmenKind.Guard),
                    new Trait(TraitCondition.Reserve, TraitEffect.NextTurnRecovery, Amount: 1, Threshold: 4),
                    "温存: 残 4 以上で次の回復 +1"),
            });

        /// <summary>Every enemy the core knows, in roster order. One for the slice; twelve after #51.</summary>
        public static readonly IReadOnlyList<EnemyDef> All = new[] { PolearmWarped };

        public static EnemyDef ById(string id)
        {
            if (id == null) throw new ArgumentNullException(nameof(id));
            foreach (var def in All)
            {
                if (string.Equals(def.Id, id, StringComparison.Ordinal)) return def;
            }
            throw new KeyNotFoundException($"Unknown enemy id \"{id}\".");
        }

        /// <summary>
        /// Builds a position-less normal enemy (roster §2) and refuses a tree that names an action the
        /// enemy does not have, so a typo in the data fails at load and not mid-battle.
        /// </summary>
        private static EnemyDef Build(
            string id,
            string name,
            int maxHp,
            int maxStamina,
            int recovery,
            IReadOnlyList<string> branchWhenPlayerNear,
            IReadOnlyList<string> branchWhenPlayerFar,
            IReadOnlyList<EnemyActionDef> actions)
        {
            var map = new Dictionary<string, EnemyActionDef>(StringComparer.Ordinal);
            foreach (var action in actions)
            {
                if (map.ContainsKey(action.Id))
                {
                    throw new ArgumentException($"Enemy \"{id}\" defines action \"{action.Id}\" twice.");
                }
                map[action.Id] = action;
            }

            foreach (var branch in new[] { branchWhenPlayerNear, branchWhenPlayerFar })
            {
                foreach (var actionId in branch)
                {
                    if (!map.ContainsKey(actionId))
                    {
                        throw new ArgumentException($"Enemy \"{id}\" branches to unknown action \"{actionId}\".");
                    }
                }
            }

            return new EnemyDef(
                id, name, maxHp, maxStamina, recovery,
                HasPosition: false, StartPosition: null,
                branchWhenPlayerNear, branchWhenPlayerFar, map);
        }
    }
}
