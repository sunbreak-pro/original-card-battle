...
public static class ProceduralArt
{
    ...
    public static Sprite White => _white ??= Make(4, 4, (x, y) => Color.white);

    public static Sprite Circle => _circle ??= Make(64, 64, (x, y) =>
    {
        float d = Dist(x, y, 32f, 32f) / 30f;
        return Alpha(Mathf.Clamp01((1f - d) * 6f));
    });

    public static Sprite SoftCircle => _softCircle ??= Make(64, 64, (x, y) =>
    {
    ...
    public static Sprite Glow => _glow ??= Make(128, 128, (x, y) =>
    {
    ...
    public static Sprite Shield => _shield ??= Make(64, 72, (x, y) =>
        Alpha(InsideShield(x / 64f, 1f - y / 72f, 0f) ? 1f : 0f));

    public static Sprite Figure => _figure ??= Make(140, 300, (x, y) => Alpha(InsideFigure(x, y, false) ? 1f : 0f));

    public static Sprite FigureWithSpear => _figureSpear ??= Make(140, 300, (x, y) => Alpha(InsideFigure(x, y, true) ? 1f : 0f));

    /// <summary>Wet cobble: value noise with a faint horizontal highlight band.</summary>
    public static Sprite FloorNoise => _floorNoise ??= Make(256, 64, (x, y) =>
    {
    ...
    public static Sprite Mist => _mist ??= Make(512, 128, (x, y) =>
    {
        const float w = 512f;
        float t = x / w;
    ...
    public static Sprite PaperGrain => _paperGrain ??= Make(256, 256, (x, y) =>
    {
    ...
    public static Sprite Triangle => _triangle ??= Make(64, 64, (x, y) => Alpha(x >= 64 - y ? 1f : 0f));

    /// <summary>Corner vignette: opaque in the corner, clear toward the centre. Rotate per corner.</summary>
    public static Sprite Vignette => _vignette ??= Make(128, 128, (x, y) =>
    {
    ...
    public static Sprite Icon(IconKind kind)
    {
        ...
        Icons[i] = Make(32, 32, (x, y) => Alpha(InsideIcon(kind, x, y) ? 1f : 0f));
        return Icons[i];
    }
