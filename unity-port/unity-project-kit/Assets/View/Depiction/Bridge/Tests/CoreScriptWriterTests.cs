...
namespace Depiction.Bridge.Tests
{
    public class CoreScriptWriterTests
    {
        ...
        /// <summary>A battle dealt in deck order, with the writer already past the opening.</summary>
        private static (BattleState State, CoreScriptWriter Writer) Begin(Position start, params CardDef[] kinds)
        {
            var setup = new BattleSetup(Enemies.PolearmWarped, Cards.BuildDeck(kinds, 1), start);
        ...
        public void Opening_ShowsBothUnits_AndKeepsTheFirstOmenHidden()
        {
            ...
            Assert.That(frame.Player.RangeGlyph, Is.EqualTo("近"));
            Assert.That(frame.Enemy.Hp, Is.EqualTo(60));
            Assert.That(frame.Enemy.HasRange, Is.False, "the polearm carries no position");
        ...
        {
            var (state, writer) = Begin(Position.Near, FiveCards);
            ...
            Assert.That(ev.After.Omen.SideGlyph, Is.EqualTo("近"), "the roster's 攻撃・近, as written (#116)");
        ...
        public void TheHandFaces_CarryTheCanonText_AndTheCoreDecidesTheLamp()
        {
            ...
            Assert.That(kesa.TraitText, Is.EqualTo("近 +5"));
            Assert.That(kesa.TraitLit, Is.True, "the player stands near");
            ...
            Assert.That(bodyCheck.TraitText, Is.EqualTo("近 重撃"));

            CardFace feint = frame.Hand.First(c => c.Name == "牽制");
            Assert.That(feint.Description, Is.EqualTo("敵に 8 ダメージ。遠間へ動く。"));
        }

        [Test]
        public void TheLamp_GoesOut_WhenThePlayerLeavesTheSide()
        {
            var (state, writer) = Begin(Position.Near, FiveCards);
            StepResult begin = TurnLoop.BeginPlayerTurn(state, NoShuffle);
            ...
            Assert.That(after.Player.RangeGlyph, Is.EqualTo("遠"));
        ...
        public void AnUnguardedHit_IsOneSlash_WithTheSettledNumbers()
        {
            ...
            Assert.That(ev.Cues[1].Text, Is.EqualTo("近 +5"));
        ...
        public void AttackThenMoveThenGuard_PlayInThatOrder()
        {
            ...
            Assert.That(move.RangeAfter, Is.EqualTo(RangeSide.Far));
            Assert.That(move.RangeGlyphAfter, Is.EqualTo("遠"));
            Cue guard = ev.Cues[4];
        ...
        [Test]
        public void EndTurn_IsThreeEvents_TurnEnd_EnemyAction_NextOmen()
        {
            var (state, writer) = Begin(Position.Near, FiveCards);
            StepResult begin = TurnLoop.BeginPlayerTurn(state, NoShuffle);
            writer.Write(begin.Events, begin.State);
            ...
            Assert.That(events.Select(e => e.Order), Is.EqualTo(new[] { 2, 3, 4 }));
            ...
            // The shove: 5 into Guard 3 → 3 blocked, 2 through, and the player is pushed far.
            ...
            Assert.That(shove.Cues[4].RangeGlyphAfter, Is.EqualTo("遠"));
            ...
            // Next omen: the enemy's 構え goes up, then the sweep is announced from the far branch.
            DepictionEvent next = events[2];
            ...
            Assert.That(next.After.Omen.KindLabel + "・" + next.After.Omen.SideGlyph, Is.EqualTo("攻撃・遠"));
            Assert.That(next.After.Omen.ValueText, Is.EqualTo("8"));
        }

        [Test]
        public void ASweepThatFindsThePlayerNear_StrikesItsBonusOff()
        {
            var kinds = new[] { CardCatalog.StepInGuard, CardCatalog.KesaCut, CardCatalog.Brace, CardCatalog.Feint, CardCatalog.Thrust };
            var (state, writer) = Begin(Position.Far, kinds);
            StepResult begin = TurnLoop.BeginPlayerTurn(state, NoShuffle);
            writer.Write(begin.Events, begin.State);
        ...
        {
            var (state, writer) = Begin(Position.Near, CardCatalog.All.ToArray());
            StepResult begin = TurnLoop.BeginPlayerTurn(state, NoShuffle);
            writer.Write(begin.Events, begin.State);
        ...
        public void TheWriter_OnlyCopiesSettledValues_EvenFromAStreamNoBattleProduced()
        {
            ...
            var hit = new List<BattleEvent>
            {
                ...
                new ActionExecuted(Actor.Enemy, Enemies.PolearmWarped.Actions["reach_thrust"], Position.Near),
